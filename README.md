...
本リポジトリは、以下のマルチパッケージ構造で構成されています。詳細なドキュメントは **[GitHub Wiki](https://github.com/altair-team/altair_framework/wiki)** をご参照ください。
...
└── docs/ / ローカルドキュメント（Wiki の事前ステージング用）
```
...
./run_nuc.sh
```
詳細な設定方法は **[Wiki: NUCセットアップガイド](https://github.com/altair-team/altair_framework/wiki/NUC%E3%82%BB%E3%83%83%E3%83%88%E3%82%A2%E3%83%83%E3%83%97%E3%82%AC%E3%82%A4%E3%83%89)** を参照してください。
...
* **[Wiki: 動作プログラム作成ガイド](https://github.com/altair-team/altair_framework/wiki/動作プログラム作成ガイド)** の内容をコピーします。
...
This repository consists of the following multi-package structure. For detailed documentation, refer to the **[GitHub Wiki](https://github.com/altair-team/altair_framework/wiki)**.
...
└── docs/ / Local documentation (for Wiki staging)
```
...
./run_nuc.sh
```
Refer to **[Wiki: NUC Setup Guide](https://github.com/altair-team/altair_framework/wiki/NUC-Setup-Guide)** for persistent port naming, passwordless SocketCAN setup, and auto-start on boot.
...
* **Programs**: Dynamically select, launch, and stop any Python script in the `user_behaviors/` directory from the WebUI. New program files are automatically detected and displayed in the dropdown menu without restarting. Script output and error logs are streamed in real-time.
...
* Copy the contents of **[Wiki: Behavior Programming Guide](https://github.com/altair-team/altair_framework/wiki/Behavior-Programming-Guide)**.
* Paste it into your AI chat interface.
* Append your target scenario to the prompt template at the bottom of the guide and send it.
* Save the generated Python code to the `user_behaviors/` directory with any filename (`*.py`). The file will be automatically detected in the WebUI Programs dropdown menu.
