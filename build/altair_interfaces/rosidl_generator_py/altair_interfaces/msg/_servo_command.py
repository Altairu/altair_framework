    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'altair_interfaces.msg.ServoCommand')
            logger.debug(
...
class ServoCommand(metaclass=Metaclass_ServoCommand):
    ...
    def __init__(self, **kwargs):
        ...
        if 'angles' not in kwargs:
            self.angles = numpy.zeros(6, dtype=numpy.uint8)
        else:
    ...
    def angles(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.uint8, \
                "The 'angles' numpy.ndarray() must have the dtype of 'numpy.uint8'"
            assert value.size == 6, \
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'angles' field must be a set or sequence with length 6 and each value of type 'int' and each unsigned integer in [0, 255]"
        self._angles = numpy.array(value, dtype=numpy.uint8)
