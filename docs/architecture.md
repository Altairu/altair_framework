...
**user_behaviors（ユーザー動作プログラム）:**
- 操縦PC側の `user_behaviors/` ディレクトリに配置されたPythonスクリプト
- `AltairBehavior` クラスを継承した、ロボットの自律動作を記述したプログラム
- WebUI 上から動的に選択・起動・停止が可能
- ファイルを追加・削除すると自動検出される（サーバー再起動不要）
- モータ制御、センサー監視、状態遷移管理など任意の制御ロジックを実装可能
...
- **動作プログラムの「動的管理」 (開発効率向上)**:
  `user_behaviors/` ディレクトリに新規プログラムを追加するだけで、即座に WebUI から実行可能になります。
  ロボット側の `behavior_runner` ノードは新規ファイル自動発見のロジックを内蔵しており、
  ユーザーは開発環境側の `user_behaviors/` ディレクトリのみを管理すればよいため、
  開発速度とイテレーション性が大幅に向上します。
