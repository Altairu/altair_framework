...
1. **デバイスIDの確認**:
   ...
   ```text
   Bus 001 Device 004: ID 1d50:606f OpenMoko, Inc. Geschwister ibsen CANable/candleLight USB CAN adapter
   ```
   - ベンダーID (`vendorId`): `1d50`
   - プロダクトID (`modelId`): `606f`
...
2. **udev ルールファイルの作成**:
   ...
   以下の1行を記述し、保存して閉じます。
   ```text
   SUBSYSTEM=="tty", ATTRS{idVendor}=="1d50", ATTRS{idProduct}=="606f", SYMLINK+="altair_can", MODE="0666"
   ```
